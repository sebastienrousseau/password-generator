...
describe("Domain: entropy-normalizer", () => {
  ...
  describe("calculateCustomEntropy", () => {
    it("should calculate ~105 bits for 16 chars from 94-char set", () => {
      const config = {
        allowedChars: "!\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~", // 94 printable ASCII chars
        length: 16,
        iteration: 1
      };
  ...
  describe("ENTROPY_CALCULATOR_REGISTRY", () => {
    it("should contain all password types", () => {
      expect(ENTROPY_CALCULATOR_REGISTRY[PASSWORD_TYPES.DICEWARE]).to.equal(calculateDicewareEntropy);
      ...
      expect(ENTROPY_CALCULATOR_REGISTRY[PASSWORD_TYPES.MEMORABLE]).to.equal(calculateMemorableEntropy);
      expect(ENTROPY_CALCULATOR_REGISTRY[PASSWORD_TYPES.QUANTUM]).to.equal(calculateQuantumEntropy);
      expect(ENTROPY_CALCULATOR_REGISTRY[PASSWORD_TYPES.HONEYWORD]).to.equal(calculateHoneywordEntropy);
      expect(ENTROPY_CALCULATOR_REGISTRY[PASSWORD_TYPES.PRONOUNCEABLE]).to.equal(calculatePronounceableEntropy);
    });
  });
  ...
  describe("validateEntropyInputs", () => {
    it("should validate correct inputs", () => {
      const result = validateEntropyInputs(
        "test-password",
