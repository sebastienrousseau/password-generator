...
describe("Domain: base64-generation", () => {
  describe("validatePositiveInteger", () => {
    ...
    it("should throw RangeError for zero", () => {
      expect(() => validatePositiveInteger(0, "count")).to.throw(
        RangeError,
        "count"
      );
    });
    ...
    it("should throw RangeError for negative numbers", () => {
      expect(() => validatePositiveInteger(-1, "length")).to.throw(
        RangeError,
        "length"
      );
      expect(() => validatePositiveInteger(-100, "size")).to.throw(
        RangeError,
        "size"
      );
    });
    ...
    it("should throw RangeError for non-integers", () => {
      expect(() => validatePositiveInteger(1.5, "value")).to.throw(
        RangeError,
        "value"
      );
      expect(() => validatePositiveInteger(0.1, "num")).to.throw(
        RangeError,
        "num"
      );
    });
  ...
  describe("BASE64_DOMAIN_RULES", () => {
    ...
    it("should have consistent encoding ratio", () => {
      expect(BASE64_DOMAIN_RULES.CHARS_PER_CHUNK / BASE64_DOMAIN_RULES.BYTES_PER_CHUNK).to.be.closeTo(4 / 3, 0.0001);
    });
  });
});
