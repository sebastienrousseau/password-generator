  ...
  describe('Console warnings', () => {
    ...
    describe('generateQuantumPassword warnings', () => {
      it('should warn when total entropy is below 256 bits', async () => {
        ...
        const totalEntropyWarning = consoleWarnStub
          .getCalls()
          .find((call) => call.args[0].includes('Total entropy'));
      ...
      it('should warn about both chunk length and total entropy when applicable', async () => {
        ...
        const chunkWarning = consoleWarnStub
          .getCalls()
          .find((call) => call.args[0].includes('below recommended minimum'));
        const totalEntropyWarning = consoleWarnStub
          .getCalls()
          .find((call) => call.args[0].includes('Total entropy'));
      ...
      it('should not warn when multiple short chunks meet total entropy target', async () => {
        ...
        const chunkWarnings = consoleWarnStub
          .getCalls()
          .filter((call) => call.args[0].includes('below recommended minimum'));
        const totalEntropyWarnings = consoleWarnStub
          .getCalls()
          .filter((call) => call.args[0].includes('Total entropy'));
