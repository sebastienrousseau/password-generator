...
function createMockStorage(options = {}) {
  ...
  const { throwOnSet = false, throwOnGet = false, throwOnRemove = false, throwOnClear = false, throwOnKey = false } = options;
...
import { WebLocalStorage, StorageKeys, storage as defaultStorage, webStorage } from "../src/adapters/web/WebLocalStorage.js";
...
describe("Onboarding - OnboardingFlow Class", function () {
  ...
  describe("OnboardingFlow callback execution on success", function () {
    it("should call onComplete with config when runOnboarding succeeds", async function () {
      ...
      const mockResult = { config: { type: "strong", length: 16 }, clipboard: true, preset: "quick" };
