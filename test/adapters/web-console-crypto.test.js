...
describe('Web Console and Crypto Adapters - Full Coverage', () => {
  ...
  describe('WebConsoleLogger', () => {
    ...
    describe('group', () => {
      ...
      it('should call groupEnd even if callback throws', () => {
        ...
        expect(() => customLogger.group('ErrorGroup', callback)).to.throw('Callback error');
  ...
  describe('WebCryptoRandom (webcrypto-random.js)', () => {
    ...
    beforeEach(async () => {
      const module = await import('../../src/adapters/web/webcrypto-random.js');
    ...
    describe('randomBytes', () => {
      ...
      it('should throw RangeError for zero size', () => {
        expect(() => randomBytes(0)).to.throw(RangeError, 'size must be a positive integer');
      });

      it('should throw RangeError for negative size', () => {
        expect(() => randomBytes(-5)).to.throw(RangeError, 'size must be a positive integer');
      });

      it('should throw RangeError for non-integer size', () => {
        expect(() => randomBytes(2.5)).to.throw(RangeError, 'size must be a positive integer');
      });

      it('should throw RangeError for string size', () => {
        expect(() => randomBytes('16')).to.throw(RangeError, 'size must be a positive integer');
      });

      it('should throw RangeError for NaN', () => {
        expect(() => randomBytes(NaN)).to.throw(RangeError, 'size must be a positive integer');
      });

      it('should throw RangeError for null', () => {
        expect(() => randomBytes(null)).to.throw(RangeError, 'size must be a positive integer');
      });
    ...
    describe('randomInt', () => {
      ...
      it('should throw RangeError for zero max', () => {
        expect(() => randomInt(0)).to.throw(RangeError, 'max must be a positive integer');
      });

      it('should throw RangeError for negative max', () => {
        expect(() => randomInt(-10)).to.throw(RangeError, 'max must be a positive integer');
      });

      it('should throw RangeError for non-integer max', () => {
        expect(() => randomInt(5.5)).to.throw(RangeError, 'max must be a positive integer');
      });

      it('should throw RangeError for string max', () => {
        expect(() => randomInt('100')).to.throw(RangeError, 'max must be a positive integer');
      });

      it('should throw RangeError for null max', () => {
        expect(() => randomInt(null)).to.throw(RangeError, 'max must be a positive integer');
      });

      it('should throw RangeError for undefined max', () => {
        expect(() => randomInt(undefined)).to.throw(RangeError, 'max must be a positive integer');
      });
    });
    ...
    describe('WebAdapters combined object', () => {
      ...
      describe('WebAdapters.storage', () => {
        ...
        it('should have async setItem method', async () => {
          const result = await indexModule.WebAdapters.storage.setItem('testKey', {
            data: 'value',
          });
        ...
        it('should have async getItem method', async () => {
          ...
          const result = await indexModule.WebAdapters.storage.getItem('getTest');
          expect(result).to.deep.equal({ value: 123 });
        });

        it('should have async getItem with default value', async () => {
          const result = await indexModule.WebAdapters.storage.getItem('nonexistent', 'default');
          expect(result).to.equal('default');
        });

        it('should have async removeItem method', async () => {
          await indexModule.WebAdapters.storage.setItem('removeMe', 'data');
          const result = await indexModule.WebAdapters.storage.removeItem('removeMe');
          expect(result).to.be.true;
        });
