...
function generateFeedback(score, patterns, dictionaries, composition) {
  ...
  if (
    patterns.some(
      (p) => p.pattern === "sequence" || p.pattern === "reverse_sequence",
    )
  ) {
  ...
  if (patterns.some((p) => p.pattern === "leet_speak")) {
    feedback.suggestions.push(
      "Predictable substitutions like @ for a are easy to guess",
    );
  }
  ...
  if (score <= 1) {
    ...
    feedback.recommendations.push(
      "Use a passphrase with multiple random words",
    );
  } else if (score <= 2) {
...
export function analyzePasswordStrength(password) {
  ...
  if (dictionaries.some((d) => d.dictionary === "common_passwords")) {
    ...
    const reductionFactor = Math.min(
      0.8,
      patternPenalty * 0.2 + dictionaries.length * 0.1,
    );
  ...
  return {
    ...
    details: {
      ...
      reductionFactors: {
        ...
        commonPassword: dictionaries.some(
          (d) => d.dictionary === "common_passwords",
        ),
      },
    },
  };
